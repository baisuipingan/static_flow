...
if [[ "$ACTIVATE_TARGET" == "both" ]]; then
  ...
  rollback_cmd+=" && sudo systemctl daemon-reload && sudo systemctl restart \"$WORKER_SERVICE\" \"$SERVICE\""
  ...
  rollback_cmd+=" && sudo systemctl daemon-reload && sudo systemctl restart \"$WORKER_SERVICE\""
fi
