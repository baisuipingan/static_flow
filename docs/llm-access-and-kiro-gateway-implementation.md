...
> 账号、代理、调度和 usage 机制仍然适用；引用 `crates/backend/src/...` 的位置请理解为
...
| 术语 | 含义 | 当前实现中的位置 |
|---|---|---|
| Public Access Page | 给外部用户看的公开说明页，不直接代理真实上游，只返回接入信息或额度快照 | `crates/frontend/src/pages/llm_access.rs`、`crates/frontend/src/pages/kiro_access.rs` |
| Gateway Key | StaticFlow 自己发放的公开或私有 key，不等于上游真实账号 token | `llm_gateway_keys` |
| Codex Account | 用于访问 ChatGPT Codex backend 的真实账号快照，可来自 `~/.codex/auth.json` 或导入后的账号池 | `crates/backend/src/llm_gateway/accounts.rs` |
| Kiro Auth Record | 用于访问 Kiro/AWS 上游的真实账号记录，保存在独立 auth 文件目录中 | `crates/backend/src/kiro_gateway/auth_file.rs` |
...
对应路由入口在 `crates/backend/src/routes.rs:59-117`。
...
项目把这部分统一收敛到了 `crates/backend/src/upstream_proxy.rs` 的 `UpstreamProxyRegistry`。
...
这个默认值定义在 `crates/backend/src/upstream_proxy.rs:27`，回退链定义在 `crates/backend/src/upstream_proxy.rs:310-329`。
...
- 路由与主流程：`crates/backend/src/llm_gateway.rs`
- 请求归一化：`crates/backend/src/llm_gateway/request.rs`
- 上游 client/runtime：`crates/backend/src/llm_gateway/runtime.rs`
- 账号池与选路：`crates/backend/src/llm_gateway/accounts.rs`
- token 刷新与 usage 拉取：`crates/backend/src/llm_gateway/token_refresh.rs`
- `/v1/models` 特殊处理：`crates/backend/src/llm_gateway/models.rs`
...
- 基础入口与 admin API：`crates/backend/src/kiro_gateway/mod.rs`
- Anthropic-compatible facade：`crates/backend/src/kiro_gateway/anthropic/mod.rs`
- 真实 upstream provider：`crates/backend/src/kiro_gateway/provider.rs`
- token 生命周期：`crates/backend/src/kiro_gateway/runtime.rs`
- 本地调度器：`crates/backend/src/kiro_gateway/scheduler.rs`
- 额度状态缓存：`crates/backend/src/kiro_gateway/status_cache.rs`
- 持久化 auth 文件：`crates/backend/src/kiro_gateway/auth_file.rs`
...
- `crates/backend/src/state.rs` 启动日志
...
这些表的 schema 统一定义在 `crates/shared/src/llm_gateway_store/schema.rs`。
...
- `crates/backend/src/upstream_proxy.rs`
- `crates/shared/src/llm_gateway_store/mod.rs`
- `crates/shared/src/llm_gateway_store/schema.rs`
- `crates/shared/src/llm_gateway_store/types.rs`

### 9.3 Codex 网关

- `crates/backend/src/llm_gateway.rs`
- `crates/backend/src/llm_gateway/request.rs`
- `crates/backend/src/llm_gateway/runtime.rs`
- `crates/backend/src/llm_gateway/accounts.rs`
- `crates/backend/src/llm_gateway/token_refresh.rs`
- `crates/backend/src/llm_gateway/models.rs`

### 9.4 Kiro 网关

- `crates/backend/src/kiro_gateway/mod.rs`
- `crates/backend/src/kiro_gateway/anthropic/mod.rs`
- `crates/backend/src/kiro_gateway/provider.rs`
- `crates/backend/src/kiro_gateway/runtime.rs`
- `crates/backend/src/kiro_gateway/scheduler.rs`
- `crates/backend/src/kiro_gateway/status_cache.rs`
- `crates/backend/src/kiro_gateway/auth_file.rs`

### 9.5 前端入口

- `crates/frontend/src/pages/llm_access.rs`
- `crates/frontend/src/pages/llm_access_guide.rs`
- `crates/frontend/src/pages/kiro_access.rs`
